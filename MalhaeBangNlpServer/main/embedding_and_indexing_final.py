...
df = pd.read_sql(
    f"""
    SELECT house_id, title, price, address, floor, deposit_type, management_fee, 
           available_from, house_num, agent_comm, agent_info, rooms_count, options, 
           posted_at, gu, dong, img_url, area_size, direction, built_date, parking, 
           building_type, house_feature, house_explanations, safety_grade, deposit, 
           monthly_rent, space, bath_count, total_floor, latitude, longitude, gpt_description 
    FROM {TABLE_NAME}
    """,
    conn
)
...
for pkl_file in pkl_files:
    ...
    for house_id, vector in id_vector_dict.items():
        ...
        doc_body = {
            ...
            "floor": int(float(row["floor"])) if pd.notna(row["floor"]) and row["floor"] != '-' else None,
