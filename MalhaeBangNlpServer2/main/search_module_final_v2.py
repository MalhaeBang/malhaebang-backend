...
ES_ID = os.getenv("ES_ID") or "elastic"
ES_PW = os.getenv("ES_PW") or "elasticteam3"
ES_INDEX = os.getenv("ES_INDEX") or "cleaned_house"
TMAP_API_KEY = os.getenv("TMAP_API_KEY")

es = Elasticsearch(ES_HOST, basic_auth=(ES_ID, ES_PW), verify_certs=False)
...
def handle_query(query):
    ...
    for i, item in enumerate(sorted_hits, 1):
        ...
        #print(f"\n🟩 [Top-{shown}, 종합 점수: {total_score:.1f}/10] (ID: {src.get('house_id', '')})")
        ...
        #print(f"🄹 주차: {'가능' if src.get('parking', 0) else '불가'}")
        ...
        if semantic_keywords:
            ...
            #print(f"💡 만족하는 옵션: {matched if matched else '없음'}")
